...
public class createNewVM {
	...
	private static String dcName = "Team19_DC_new";
	static String ip;
	...
	public  static void doClone() throws Exception{	
		...
		try
		{
			...
			ServiceInstance si = new ServiceInstance(	new URL("https://130.65.133.74/sdk"),SJSULAB.getVmwareLogin(), 
					SJSULAB.getVmwarePassword(), true);
			Folder	rootFolder = si.getRootFolder();
			VirtualMachine	vm = (VirtualMachine) new InventoryNavigator(rootFolder).searchManagedEntity("VirtualMachine", "T19-FinalTemplate");// Template Name
			
			Datacenter dc = (Datacenter) new InventoryNavigator(rootFolder).searchManagedEntity("Datacenter", dcName);
			ManagedEntity host =new InventoryNavigator(rootFolder).searchManagedEntity("HostSystem", ip);
