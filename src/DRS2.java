...
public class DRS2 {
	...
	static String ip= "130.65.133.78";
	/**
	 * @param args
	 */
	public DRS2(){
			...
			try {
				si =  new ServiceInstance(new URL(Config.getvCenterURL()), Config.getvCenterUsername(), Config.getPassword(),true);
	...
	public void addingNewHost()
	{
		...
		{	
			...
			hs.userName ="root";
			hs.password = "12!@qwQW";
			hs.managementIp = "130.65.133.74";//130.65.133.60
	...
	public void liveMigrateVM(String new_host_ip, String vmname)throws Exception{   //ip of host where to migrate and vm object of machine which needs to be migrate
		si= new ServiceInstance(new URL(Config.getvCenterURL()),
				Config.getvCenterUsername(), Config.getPassword(), true);
		rootFolder = si.getRootFolder();
		ip=new_host_ip;
		try{
